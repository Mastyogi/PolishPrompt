...
// Use a flexible regex to find the augmentReq function regardless of whitespace differences
const augmentReqRegex = /function\s+augmentReq\s*\(\s*cfReq\s*,\s*ctx\s*\)\s*\{[^}]*\}[^}]*\}/s;

const match = serverIndex.match(augmentReqRegex);
...
if (!match) {
  ...
  const patched = serverIndex.replace(match[0], replacement);
